...
// 路由表里进行数据的处理 传送、渲染  添加、修改、删除、
// 一、引入
// 1、引入exppress模块
// 2、引入路由（相当于从后端拿取数据的一个通道）
// 3、引入schema模型数据（../sql/production  文件地址）
...
// 二、渲染商品管理页的样式
// 确定传输数据的方式并在里面写回调函数
// 没找到报错  ----找到合适production模板的数据将其通过rend()的方式在pro(商品管理的页面)，返回数据并将index改为1代表商品管理页面
/* GET home page. */
router.get("/", function (req, res, next) {
  ...
  production.find({},(err,data)=>{
      ...
      res.render("pro",{//pro是pro.ejs
        index:1,//传那个就在sidebar中亮起
        data:data
      })
  })
...
// 2、添加数据：
// a、确定传输方式
router.post("/addAction", function (req, res, next) {  //addAction连接操作的节点
  let obj = req.body;
  //b、限定写入数据传输的格式
...
// c、限定传出schema模型数据
  production.insertMany(obj,(err,data)=>{
       ...
       console.log(data)
       res.redirect("/pro");  //确定数据渲染到pro商品管理的pro.ejs中
  })
